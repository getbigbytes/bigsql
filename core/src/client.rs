...
impl APIClient {
    ...
    async fn check_presign(&mut self) -> Result<()> {
        match self.presign {
            PresignMode::Auto => {
                if self.host.ends_with(".getbigbytes.com") || self.host.ends_with(".databend.cn") {
                    self.presign = PresignMode::On;
                } else {
                    self.presign = PresignMode::Off;
                }
            }
            PresignMode::Detect => match self.get_presigned_upload_url("@~/.bigsql/check").await {
...
mod test {
    ...
    async fn parse_dsn() -> Result<()> {
        let dsn = "databend://username:password@app.getbigbytes.com/test?wait_time_secs=10&max_rows_in_buffer=5000000&max_rows_per_page=10000&warehouse=wh&sslmode=disable";
        let client = APIClient::from_dsn(dsn).await?;
        assert_eq!(client.host, "app.getbigbytes.com");
        assert_eq!(client.endpoint, Url::parse("http://app.getbigbytes.com:80")?);
