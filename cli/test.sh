...
cargo build --bin bigsql

case $TEST_HANDLER in
"flight")
	echo "==> Testing Flight SQL handler"
	export BIGSQL_DSN="databend+flight://${DATABEND_USER}:${DATABEND_PASSWORD}@${DATABEND_HOST}:8900/?sslmode=disable"
	;;
"http")
	echo "==> Testing REST API handler"
	export BIGSQL_DSN="databend://${DATABEND_USER}:${DATABEND_PASSWORD}@${DATABEND_HOST}:${DATABEND_PORT}/?sslmode=disable&presign=on"
	;;
*)
...
export BIGSQL="${CARGO_TARGET_DIR}/debug/bigsql"
...
for tf in cli/tests/*.{sql,sh}; do
	...
	if [[ $tf == *.sh ]]; then
		...
		"${BIGSQL}" --output tsv <"${tf}" >"cli/tests/${suite}.output" 2>&1 || true
	fi
...
for tf in cli/tests/"$TEST_HANDLER"/*.{sql,sh}; do
	...
	if [[ $tf == *.sh ]]; then
		...
		suite=$(basename "${tf}" | sed -e 's#.sql##')
		"${BIGSQL}" --output tsv <"${tf}" >"cli/tests/${TEST_HANDLER}/${suite}.output" 2>&1 || true
	fi
